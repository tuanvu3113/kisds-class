...
class home_imageModel extends CI_Model {
    ...
    function change_is_publish($id, $status) {
         ...
         * set only record have is_publish equal 1
          if (status == 1) {
          $sql = "UPDATE `gdt_home` SET `is_publish` = '0';";
          $this->model->executeQuery($sql);
          }
         */
        $sql = "UPDATE `gdt_home` SET `is_publish` = '" . $status . "' WHERE `id` = " . $id . ";";
        $this->model->executeQuery($sql);
    }
    ...
    function mysave($id, $data) {
        ...
        try {
            if (empty($id)) {
                $check = $this->db->select("id")->from('gdt_home')->where("title", $data['title'])->where("is_delete", 0)->count_all_results();
                if ($check > 0) {
                    $rs["status"] = "exist";
                ...
                $sql = "
                    INSERT INTO gdt_home (title, title_vn, link, label, label_vn, picture, picture_mobile, content, content_vn, is_publish, json_pictures, user_create, time_create, area, user_update) VALUES ('$title', '$title_vn', '$link', '$label', '$label_vn', '$picture', '$picture_mobile', '$content', '$content_vn', $is_publish, '$json_pictures', 1, '$time_create', $area, 1)
                ";
                $this->model->executeQuery($sql);
            } else {
    ...
    function mydelete($idList) {
        $sql = "UPDATE gdt_home SET is_delete=1 WHERE id IN (" . $idList . ") AND id > 0 AND is_delete = 0";
        $this->model->executeQuery($sql);
    }
}
