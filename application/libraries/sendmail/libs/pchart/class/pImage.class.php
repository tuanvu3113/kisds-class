 ...
 class pImage extends pDraw
  {
   ...
   /* Render the picture to a web browser stream */
   function strokeFile($FileName, $FilePath)
	{
		if ( $this->TransparentBackground ) { 
			imagealphablending($this->Picture,false); imagesavealpha($this->Picture,true); 
		}
		imagepng($this->Picture, $FilePath.$FileName);
    }
   /* Automatic output method based on the calling interface */
   function autoOutputFile($FileName="output.png", $FilePath = '')
    {
     if (php_sapi_name() == "cli")
      $this->renderFile($FileName);
     else
		return $this->strokeFile(false, $FileName, $FilePath);
    }
	#end region them vao
