...
class XLSXWriter
{
	...
	public function writeSheetHeader($sheet_name, array $header_types,$setrow=0, $suppress_row = false)
	{
        ...
        if (!$suppress_row)
        {
			...
			foreach ($header_row as $k => $v) {
				$this->writeCell($sheet->file_writer, $setrow, $k, $v, $cell_format_index = '0');//'0'=>'string'
			}
	...
	public function writeSheetRow($sheet_name, array $row,$setrow)
	{
		...
		$column_count=0;
		foreach ($row as $k => $v) {
			$this->writeCell($sheet->file_writer, $sheet->row_count+ $setrow, $column_count, $v, $sheet->columns[$column_count]);
			$column_count++;
		}
		$sheet->file_writer->write('</row>');
		$sheet->row_count++;
	...
	public function writeSheet(array $data, $sheet_name='', array $header_types=array(),$setrow = 0)
	{
		$sheet_name = empty($sheet_name) ? 'Sheet1' : $sheet_name;
		$data = empty($data) ? array(array('')) : $data;
		if (!empty($header_types))
		{
			$this->writeSheetHeader($sheet_name, $header_types,$setrow);
		}
		foreach($data as $i=>$row)
		{
			$this->writeSheetRow($sheet_name, $row, $setrow);
		}
