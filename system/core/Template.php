...
class CI_Template {
   ...
   var $folder_template = 'themes';
   var $subfolder_template = '';
   var $file_template = 'index';
   var $path;

   /**
	 * Constructor
	 *
	 * Loads template configurationuration, template regions, and validates existence of
	 * default template
	 *
	 * @access	public
	 */

   function __construct($subfolder_template = '', $active_template = '')
   {
      $this->subfolder_template = $subfolder_template;
      // Copy an instance of CI so we can use the entire framework.
      $this->CI =& get_instance();

      // Load the template configuration file and setup our master template and regions
      include(APPPATH.'config/template'.EXT);
      if (isset($template)){
         $this->configuration = $template;
		 if(!empty($active_template)) $this->configuration['active_template'] = $active_template;
         $this->set_template($template['active_template']);
      }
      $this->path = $this->folder_template .'/'. $this->subfolder_template .'/'. $this->configuration['active_template'] .'/';
   }
   ...
   function set_template($group){
      ...
      {
         $this->template = $this->configuration[$group];
      }
      else
      {
   ...
   function set_master_template($filename = '', $file = true){
      if(!$file){
		if(!strrpos($filename,'.')) $filename.=EXT;
	  	$path = FCPATH .DS. $filename;
	  }
	  else{
		  if(empty($filename)) $filename = $this->file_template;
		  if(!strrpos($filename,'.')) $filename.=EXT;
		  $path = FCPATH . $this->folder_template .DS. $this->subfolder_template .DS. $this->configuration['active_template'] .DS. $filename;
	  }
      if (!file_exists($path))
      {
         show_error('The filename provided does not exist in <strong>'. FCPATH .DS. 'themes'.DS. $filename) or file_exists(FCPATH .DS.'themes'.DS. $filename . EXT.'</strong>. Remember to include the extension if other than ".php"');
      }
      else
      {
      	$this->master = $this->subfolder_template .DS. $this->configuration['active_template'] .DS. $filename;
      }
   }
   ...
   function add_template($group, $template, $activate = FALSE)
   {
      if ( ! isset($this->configuration[$group]))
      {
         $this->configuration[$group] = $template;
         if ($activate === TRUE)
         {
   ...
   /**
    * Initialize class settings using configuration settings
    *
    * @access  public
    * @param   array   configurationuration array
    * @return  void
    */
   ...
   function initialize($props)
   {
      ...
      if (isset($props['template'])
         && (file_exists(FCPATH .DS. 'themes'.DS. $props['template']) or file_exists(FCPATH .DS. 'themes'.DS. $props['template'] . EXT)))
      {
   ...
   function write_view($region, $view, $data = NULL, $overwrite = FALSE)
   {
      ...
      if (count($args) > 1)
      {
         ...
         {
            if (file_exists(FCPATH .DS. 'themes'.DS. $suggestion . EXT) or file_exists(FCPATH .DS. 'themes'.DS. $suggestion))
            {
   ...
   function render($region = NULL, $buffer = FALSE, $parse = FALSE){
      ...
      // Build the output array
         ...
         else
         {
            ...
            $output = $this->CI->load->template($this->master, $this->output, $buffer);
         }
      }
